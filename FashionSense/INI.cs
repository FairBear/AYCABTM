...
namespace FashionSense
{
	public class INI
	{
		...
		public INI Combine(INI ini, string[] ignoreSections = null, Dictionary<string, string> ignoreKeys = null, bool overwrite = false)
		{
			foreach (var section in ini.Sections())
				if (ignoreSections == null || !ignoreSections.Contains(section))
					foreach (var key in ini.Keys(section))
						if (ignoreKeys == null || !ignoreKeys.Contains(new KeyValuePair<string, string>(section, key)))
							if (overwrite || !Has(section, key))
								Add(section, key, ini.Get(section, key));
		...
		public List<string> GetStringList(string section, string key, List<string> defaultValue = null)
		{
			...
			if (value != null && value.Length > 0 && Util.String.Parse(value, out List<string> result))
				return result;

			return defaultValue;
		}

		public HashSet<string> GetStringHashSet(string section, string key, HashSet<string> defaultValue = null)
		{
			var value = Get(section, key);

			if (value != null && value.Length > 0 && Util.String.Parse(value, out HashSet<string> result))
				return result;
